class bluditAjax {
	...
	userLogged(callBack) {
		...
		console.log("[INFO] [Baseless AJAX] [userLogged()] Checking if the user is logged.");
		...
		ajaxRequest.done(function (response, textStatus, jqXHR) {
			console.log("[INFO] [Baseless AJAX] [userLogged()] The user is logged.");
		});

		ajaxRequest.fail(function (jqXHR, textStatus, errorThrown) {
			// The fail is produced by admin.php when the user is not logged the ajax request is not possible and returns 401
			console.log("[INFO] [Baseless AJAX] [userLogged()] The user is NOT logged.");
			if (jqXHR.status==401) {
