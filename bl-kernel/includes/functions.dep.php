...
function buildPagesByCategory() {
	$key = url()->slug();
	return buildPagesFor( 'category', $key, false );
}

// This function is only used from the rule 69.pages.php, DO NOT use this function!
function buildPagesByTag() {
	$key = url()->slug();
	return buildPagesFor( 'tag', false, $key );
}
...
function buildPagesFor( $for, $cat = false, $tag = false ) {
...
function changePluginsPosition( $list ) {
