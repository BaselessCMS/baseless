 ...
 * Start the session
 *
 * If the session is not possible to start
 * the admin area is not available.
 */
Session :: start( $site->urlPath(), $site->isHTTPS() );
if ( Session :: started() === false ) {
	exit( 'CMS session initialization failed.' );
}
...
if ( $layout['slug']==='ajax' ) {
	...
	if ( $url->notFound() || ! $login->isLogged() || ( $url->slug() === 'login' ) ) {
		...
		/**
		 * Generate the tokenCSRF for the user not logged,
		 * when the user log-in the token will be change.
		 */
